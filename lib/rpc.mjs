...
// Safety net so a call can never hang forever if the daemon accepts the
// request but never answers (a daemon bug). A dying daemon closes the socket,
// which rejects pending calls immediately via handleClose.
...
class HubRpcClient {
  ...
  call(method, params = {}) {
    if (this.closed) {
      return Promise.reject(new Error("Vanzi hub connection is closed"));
    }
    const id = this.nextId;
    this.nextId += 1;
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        if (this.pending.delete(id)) {
          reject(new Error(`Vanzi hub call timed out: ${method}`));
        }
      }, CALL_TIMEOUT_MS);
      timer.unref?.();
  ...
  // The socket closed (daemon exited or crashed mid-call): reject every
  // in-flight call so callers surface an error instead of hanging forever.
  handleClose() {
    this.closed = true;
    const error = new Error("Vanzi hub connection closed");
    for (const pending of this.pending.values()) {
      clearTimeout(pending.timer);
      pending.reject(error);
    }
