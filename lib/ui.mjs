...
class PopupUi {
  ...
  rawInputLayout(session = this.rawInput) {
    ...
    // Flat chrome: divider + footer. Boxed chrome: top rule, then either the
    // footer or the autocomplete dropdown (no bottom border: half-box style).
    const chromeRows = boxed ? (dropdownRows ? 1 : 2) : 2;
    ...
    const boxBottomRow = null;
    const dropdownRow = dropdownRows ? inputPadBottomRow + inputPadRows : null;
    const footerRow = dropdownRows ? null : inputPadBottomRow + inputPadRows;
  ...
  boxedComposerTitle() {
    ...
    // Provider segment follows the rule color (theme accent when configured);
    // only the status glyph+word keeps its own semantic color.
    const seq = this.composerBorderSeq();
    const reset = colors.reset || "";
    const provider = `${seq}${providerIconFor(chat.provider, chat)} ${compactProviderLabel(
  ...
  renderBoxedComposer(session, layout) {
    ...
    if (layout.footerRow !== null) {
      // Wrapped-input overflow counters join the meta line (the bottom border
      ...
      const overflow = overflowParts.length ? ` ${c("dim", `· ${overflowParts.join(" · ")}`)}` : "";
      painter
        .to(0, layout.footerRow)
        .clearLine()
        .text(fitAnsiLine(`${this.composerMetaLine(this.composerFooter())}${overflow}`, safeColumns));
    }
  ...
  composerContextLabel() {
    ...
    if (typeof used === "number" && typeof size === "number" && size > 0) {
      ...
      // Compact while comfortable ("8%"); the full used/size detail appears
      // once the window fills up and the number starts to matter.
