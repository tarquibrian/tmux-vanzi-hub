...
class PopupUi {
  ...
  async adoptChatInPane(openFn) {
    if (process.env.TMUX_PANE) {
      spawnSync(
        "tmux",
        ["set-window-option", "-t", process.env.TMUX_PANE, "-q", "@vanzi_hub_action", "open"],
        { stdio: "ignore" },
      );
    }
    return openFn();
  ...
  inputHint(line) {
    ...
    // slides the composer down. Empty input advertises the ← menu / Tab mode
    // gestures; while typing a plain message it shows a keyboard tip instead.
    if (!line) {
      if (!this.currentChat?.id) return "";
      const modes = modeEntries(this.currentChat?.modes);
      if (modes.length >= 2) {
        const label = valueLabel(this.currentChat?.mode) || this.currentChat?.mode || "mode";
        return `← menu  ·  ⇥ / ⇧⇥ mode · ${label}`;
      }
      return "← menu";
    }
    if (!line.startsWith("/") || line.startsWith("//")) {
      return this.currentChat?.id ? "Enter sends · Alt+Enter for a newline · Ctrl+O menu" : "";
    }
  ...
  async showChangesPicker() {
    ...
    const picked = await this.interactivePick({
      ...
      hint: "↑↓ move · Enter show diff · Esc cancel",
      items,
    });
